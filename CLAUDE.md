...
`pipeline/evaluate.py` runs the **real** prompt path — `enrich.classify()` and `enrich._rate_stances()`, not copies — against ~46 hand-labelled cases in `evals/cases/`, split into five files by what they probe: `extraction_core` (ordinary items in de/fr/pl/en/et/lv), `format_naming` (the v6 claim, including the city of Weimar and a minister called Weimer as traps), `irrelevant` (must land all-flags-false, including two Weimar Triangle members opening cultural institutes together), `stance_scale` (the rubric plus expected omissions), and `goal_discrimination` (the v7 premise: one topic, two groupings, two different right answers). Each case embeds a **verbatim snapshot** of the event body rather than a path into `data/events/`, so a data refresh can never silently change what was measured.

Labels are hand-written and reviewable — every non-obvious one carries a `notes:` line saying why. Two label conventions matter:

- **`null` means "expect this topic omitted"**, not "expect 0". This is the only direct measurement of the prompt-v8 rule.
- **`expect.relevant` is derived, never invented.** It is whatever `_grouping_relevance()` produces from the case's own `actors`/`explicit_formats`/`topics`, and `test_case_relevance_labels_are_derived_not_invented` fails if a hand-edit leaves it stale. Every (relevant grouping × topic that grouping tracks) pair must carry a label, since that is exactly the set of ratings the pipeline will ask for.

**`evals/README.md` is the metric reference** — what each metric divides by what, how many decisions it rests on, and the noise floor that follows. Read it before interpreting a run; the summary below is an index, not a definition.

Metrics fall into three groups. **Classification**: `relevance_accuracy` (scored per grouping, not per case — five booleans, and the flag is what decides whether an event reaches the site at all), `actors_exact`/`actors_f1`, `formats_exact`, `topics_f1`. **Stance**: `stance_exact`, `stance_within_1`, `stance_mae`, `sign_agreement`, `abstention_recall`/`abstention_precision`, and `goal_discrimination`. **Mechanical** (label-free, computable on any output): `evidence_verbatim` — is the quote actually in the source text, which `_clean_evidence` does not check — plus `evidence_goal_copy`, `retry_rate`, `parse_failure_rate`.

Two things the scoring is deliberate about. A labelled pair that classification never surfaced is counted as `stance_coverage` loss and **excluded** from the stance metrics, so a classification miss can't be credited as a correct abstention. And `--repeats N` reports a **flip rate** — the fraction of decisions that differed across repeats — next to every metric, because a delta smaller than the flip rate is noise, not movement. `--stance-forced` rates the labelled topics regardless of predicted relevance, isolating rubric quality from classification quality when iterating on the rubric.

**What the first measured run found** (prompt v8, gemma4 via Ollama Cloud, 46 cases × 3 repeats, flip rate **0.042** — so anything above ~4pp is real movement):

| | |
|---|---|
| `relevance_accuracy` 0.943, `actors_f1` 0.947 | Classification is solid. The relevance rule is doing its job. |
| `formats_exact` 0.848 | v6's `explicit_formats` clarification broadly holds; ~15% still wrong. |
| `stance_exact` 0.598 but `stance_within_1` 0.957 | **The rubric's five points are finer than the model can actually resolve.** Direction is nearly always right, the exact step rarely is. Treat a single event's ±1 as noise; cluster means are the meaningful unit. |
| `abstention_recall` **0.220**, `abstention_precision` 1.000 | **The v8 instruction is largely ineffective.** When the model does omit it is always right, but it fails to omit in ~78% of the cases where it should — it reaches for a quote that is *in the text* but says nothing about *this grouping's goal*, and rates on that. `_rate_stances()`'s evidence-less-0 net only catches the subset that comes back with no quote at all. |
| `goal_discrimination` 0.583 | v7's premise — that one topic asks a different question of each grouping — holds only ~58% of the time. This is the `uk_fcdo` copy-paste defect, quantified. |
| `evidence_verbatim` 0.941 | ~6% of stored quotes are paraphrases rather than quotes, which `_clean_evidence` never checked for. |
