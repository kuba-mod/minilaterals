...
- **`docs/index.html`** (the hub page, only emitted when `SITE_BASE_PATH` is set): one card per minilateral grouping — Weimar Triangle is live and links into `{SITE_BASE_PATH}/`, the rest (`HUB_GROUPINGS` in `render.py`) are static "coming soon" placeholders pending their own ingesters/render targets. The Weimar card's statement count and alignment badge are computed from the same data as the tracker itself (`weekly_counts`, `topic_weekly["overall"]`), not hardcoded. A card's `topics` (the tag chips) must each have a stated, agreed objective behind them, documented on that grouping's entry in `data/groupings.yaml` — analytical framings a grouping's members never signed up to ("BRI alternative", "sovereigntism") don't qualify. `tests/test_hub_goals.py` fails if the two drift apart, so a new card or a retagged one needs both edited together.
...
| `data/groupings.yaml` | Every minilateral: members, goal, and the instrument that goal was agreed in (`agreed.instrument`/`date`/`level`/`url`), plus the basis for each hub card tag. Entries carrying `topics` are **tracked by the pipeline** — their members/topics are the actor and issue-area vocabulary; the rest are hub placeholders invisible to it. `tests/test_hub_goals.py` pins the tracked set and keeps `tags` in lockstep with `HUB_GROUPINGS` |
