...
| `data/grouping_goals.yaml` | Format-level goals: one entry per hub card, each stating what the grouping exists to do and the instrument its members agreed that in (`agreed.instrument`/`date`/`level`/`url`), plus the basis for every card tag. Not read by the pipeline — it's the editorial check on `HUB_GROUPINGS`, kept in lockstep by `tests/test_hub_goals.py` |
