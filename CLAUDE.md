...
Two independent systems touch the built site, and they don't automatically agree.

- **GitHub Actions** (`render.yml`) runs `pipeline.render` on every push, but only *commits* `docs/**` back when the push is to `main`. A feature-branch push still renders — the result is uploaded as a downloadable `site` artifact — but the branch's own `docs/` in git is left untouched.
- **Cloudflare Workers** (`wrangler.jsonc`, Static Assets) deploys from whatever `docs/` is committed on the pushed branch, via its own Git-integration build in the Cloudflare dashboard. It does not run `pipeline.render` on its own — `scripts/cf-build.sh` does that, as the dashboard's configured build command, immediately before Cloudflare's `wrangler versions upload` (branches) / `wrangler deploy` (`main`) step.
- **The gotcha this creates**: a PR that only touches `pipeline/**` / `pipeline/templates/**` — the normal case, per `render.yml` above — has no rendered `docs/` commit of its own. Its Cloudflare *branch preview* will only reflect the change if the dashboard build command is set to run `scripts/cf-build.sh`; otherwise Cloudflare just redeploys the stale `docs/` inherited from wherever the branch forked off `main`, silently showing the old site.
...
- **`SITE_BASE_PATH`**: the env var `render.py` reads (default `""`) to prefix every internal link and asset URL. Both `render.yml` and `scripts/cf-build.sh` set it to `/weimar-triangle`, matching the production route — which is why the rendered tree is `docs/weimar-triangle/index.html`, not `docs/index.html`.
- **`docs/_redirects`**: a static, hand-committed file (not generated by `render.py`) using Cloudflare's `_redirects` convention — `/ → /weimar-triangle/` (301) — so hitting the bare domain/subdomain root lands on the site instead of 404ing against a `docs/index.html` that doesn't exist.
