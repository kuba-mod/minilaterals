...
| `data/groupings.yaml` | **The one config file.** `topic_goals` holds the per-issue-area reference sentence each stance is rated against (was `data/goals.yaml`); `groupings` holds every minilateral — members, goal, the instrument that goal was agreed in (`agreed.instrument`/`date`/`level`/`url`), the basis for each hub card tag, and the card's own name/member_names/blurb. Entries carrying `topics` are **tracked by the pipeline**; the rest are hub placeholders invisible to it. Read by `enrich.py`, `validate.py` and `render.py` |
