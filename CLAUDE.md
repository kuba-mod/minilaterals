...
- **`docs/{grouping}/index.html`** (one per grouping without a tracker, alongside the hub): the tracker's own chrome from `base.html`, with the panels it will fill shown empty rather than faked. Everything factual on it — purpose, instrument, members, per-tag goals — comes from `data/groupings.yaml`, so there is no placeholder prose to drift. `base.html` gained `site_title`/`site_tagline`/`hdr_meta`/`footer` blocks so a non-Weimar page can reuse it. URL path is the hub slug with underscores swapped for hyphens (`us_japan_rok` → `/us-japan-rok/`); `base_path` is `""` for these since they sit at the domain root like the hub.
